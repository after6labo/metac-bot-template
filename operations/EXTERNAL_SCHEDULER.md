# 定期起動の補助スケジュール（本人による初回設定が必要）

## 現在の状態

2026-09-25: コードを準備。Google Apps Scriptへの設置・認可・トークン設定は未実施。
このファイルがGitHubにあるだけでは有効になりません。20分間隔への改善はまだ未検証です。

GitHubの全11回の履歴で、定期実行の間隔は193分30秒・183分17秒でした。
最新ジョブは作成2秒後に起動し約67秒で終了しており、Botの長時間処理ではありません。
...
少なくとも3回の補助起動と、4時間分の実際の問題確認時刻を計測します。
